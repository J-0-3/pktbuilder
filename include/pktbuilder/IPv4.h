...
namespace pktbuilder {
    namespace IPv4ProtocolNumber {
        enum IPv4ProtocolNumber : uint8_t {
            HOPOPT = 0,
            ICMP = 1,
            IGMP = 2,
            GGP = 3,
    ...
    class IPv4Packet: public IntermediaryLayer {
        ...
        uint8_t ttl{};
        uint8_t protocol = IPv4ProtocolNumber::HOPOPT;
        std::vector<IPv4Option> options;
    public:
        IPv4Packet() = default;
        explicit IPv4Packet(ipv4_addr_t destination_ip,
                            ipv4_addr_t source_ip = { 0, 0, 0, 0 },
                            uint8_t protocol = 0);
        IPv4Packet(ipv4_addr_t destination_ip, ipv4_addr_t source_ip,
